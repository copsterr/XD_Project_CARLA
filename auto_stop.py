"""
This script is used to demonstrate autonomous driving with auto stopping when
the vehicle detects an obstacle in front of itself.
"""
...
WRITE_VID  = False
VID_NAME   = "autodriving_noObstacle.avi"
WRITE_FILE = True
...
try:
    ...
    while not GameDone:
        ...
        if WRITE_FILE:
            f.write(f"{detector_notify['front']},")

        _velocity = vehicle.get_velocity()
        _velocity = np.array([_velocity.x, _velocity.y, _velocity.z])
        _absolute_velocity = round(np.linalg.norm(_velocity, ord=2), 3)

        if WRITE_FILE:
            f.write(str(_absolute_velocity) +"\n")
