"""
This script is used to demonstrate autonomous driving.
This program utilizes Lane Detect for keeping the vehicle within a lane and
also utilizes Obstacle detector to detect any obstacles around the Vehicle.
The lane changing algorithm is hard-coded and there's no path planning in the
current program yet.
"""
...
# custom libraries
from ds2_controller import DS2_Controller # dualshock2 controller for sony playstation
...
CAR_SPAWN_POINT = carla.Transform(carla.Location(x=-30, y=204, z=1))
WRITE_VID  = False
...
# setup write file
if WRITE_FILE:
    f = open("pid_resp99_steering.csv", "w")
...
try:
    ...
    while not GameDone:
        ...
        try:
            ...
            # anti windup and add deadzone by saturation
            if err > 1:
                err = 0.8
        ...
        if WRITE_FILE:
            f.write(str(detector_notify["front"]) + "," + current_lane + "\n")
